...
export default function DashboardChiefNursing() {
  ...
  const fetchReports = async () => {
    ...
    try {
      const res = await fetch(
        "https://safe-nurse-backend.vercel.app/api/laporan/kepala_ruangan",
        {
          method: "GET",
          headers: {
  ...
  const handleTolak = async () => {
    ...
    try {
      const res = await fetch(
        `https://safe-nurse-backend.vercel.app/api/laporan/reject/${reportId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            catatan: catatan, // alasan penolakan
          }),
        }
      );
      ...
      console.error("❌ Error tolak:", err.message);
